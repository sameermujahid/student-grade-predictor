...
def sanitize_df(df: pd.DataFrame):
    ...
    # Drop object type columns before showing in Streamlit
    ...
    if df.isnull().any().any():
        st.warning("⚠️ Some inputs were coerced to NaN.")
...
def prepare_example_df():
    ...
    example = {
        ...
        "freetime": 3, "goout": 2, "Dalc": 1, "Walc": 1, "health": 5, "absences": 4,
        "nursery": "yes", "G1": 14, "G2": 15, "school": "GP"
    }
    df = pd.DataFrame([example])
    bin_map = {"yes": 1, "no": 0, "F": 0, "M": 1, "U": 1, "R": 0, "LE3": 0, "GT3": 1, "T": 1, "A": 0}
    for col in df.columns:
        if df[col].iloc[0] in bin_map:
...
# =====================================================
    ...
    with col1:
        ...
        Medu = st.selectbox("Mother's education", [0, 1, 2, 3, 4], 2)
        Fedu = st.selectbox("Father's education", [0, 1, 2, 3, 4], 2)
        ...
        reason = st.selectbox("Reason for school", ["home", "reputation", "course", "other"], 2)
    ...
    with col2:
        ...
        paid = st.selectbox("Paid classes", ["yes", "no"], 1)
        activities = st.selectbox("Activities", ["yes", "no"], 0)
        higher = st.selectbox("Higher education", ["yes", "no"], 0)
        internet = st.selectbox("Internet access", ["yes", "no"], 0)
        romantic = st.selectbox("Romantic", ["yes", "no"], 1)
        ...
        health = st.selectbox("Health", [1, 2, 3, 4, 5], 4)
    ...
    for col in df_input.columns:
        if df_input[col].iloc[0] in bin_map:
            df_input[col] = df_input[col].map(bin_map)
